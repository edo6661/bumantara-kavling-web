    ...
    <div className="space-y-8">
        ...
        <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl shadow-sm flex items-start gap-4 text-amber-800">
          ...
          <div>
            ...
            <p className="text-sm font-medium leading-relaxed">
              Anda belum di-approve sebagai agent. Anda harus melengkapi berkas-berkas dokumen di bawah ini.
              Setelah dokumen lengkap, silakan <strong>hubungi admin untuk approval</strong> dan validasi berkas.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-600 p-8 rounded-2xl shadow-lg text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          {isPerusahaan && perusahaan ? (
            <>
              <h2 className="text-3xl font-black mb-1">{perusahaan.nama}</h2>
              <p className="text-blue-100 font-medium tracking-wide">
                PIC: {agentData.nama} · NIK: {agentData.nik}
              </p>
            </>
          ) : (
            <>
          ...
          {isPerusahaan && perusahaan && (
            <div className="bg-white rounded-2xl border border-blue-100 shadow-sm p-6">
              <h3 className="font-bold text-slate-800 mb-4 border-b border-slate-100 pb-4 flex items-center gap-2">
                <Building2 size={16} className="text-blue-600" /> Data Perusahaan
              </h3>
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold text-slate-800">{perusahaan.nama}</p>
                </div>
                <div>
            ...
            <div className="space-y-4 text-sm">
              {isPerusahaan && (
                <div>
                  ...
                  <p className="font-semibold text-slate-800">{agentData.nama}</p>
                </div>
              )}
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">NIK</p>
                <p className="font-semibold text-slate-800 font-mono">{agentData.nik}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">
                  No WhatsApp
                </p>
                <p className="font-semibold text-slate-800">{agentData.noHp}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">
                  Email Login
                </p>
                <p className="font-semibold text-slate-800">{agentData.email}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">
                  Alamat Lengkap
                </p>
                <p className="font-medium text-slate-600 leading-relaxed">{agentData.alamat || '-'}</p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <h3 className="font-bold text-slate-800 mb-6 border-b border-slate-100 pb-4 flex items-center gap-2">
              ...
              {isPerusahaan ? 'Kelengkapan Berkas Perusahaan' : 'Upload Kelengkapan Berkas'}
            </h3>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {documentFields.map(({ key, label }) => {
                const fileUrl = agentData[key as keyof AgentData] as string | null | undefined;
                return (
                  <div
                    key={key}
                    className="flex flex-col gap-3 p-4 border rounded-xl bg-slate-50/50 hover:bg-slate-50 transition-colors shadow-sm"
                  >
                    ...
                    {agentData.penjualan.map((sale: any) => (
                      <tr key={sale.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-4 text-center">
                          <span
                            className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${sale.status === 'LUNAS' ? 'bg-green-100 text-green-700' : sale.status === 'BATAL' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}`}
                          >
                            {sale.status}
                          </span>
                        </td>
        ...
        <form onSubmit={handleAccountSubmit} className="space-y-4">
          <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 mb-4">
            <p className="text-xs text-slate-600 leading-relaxed font-medium">
              Anda dapat memperbarui email untuk login dan password Anda di sini. Kosongkan kolom password jika tidak
              ingin mengubahnya.
            </p>
          </div>
          <Input
