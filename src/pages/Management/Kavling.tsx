...
// 1. Interface Data Super Lengkap
interface KavlingData {
  id: string;
  // --- Informasi Dasar ---
  perumahan: string; status: string; lantai: string; blok: string; unit: string; tipe: string;
  luasBangunan: string; lokasiStrategis: string; tanggalAkadPpjb: string; akadPpjb: string;
  tanggalAkadAjbPpat: string; tanggalPembayaranPph: string; tanggalPembayaranBphtb: string; pembiayaan: string; sp3r: string;

  // --- Harga Jual ---
  harga: number; lebihTanah: number; biayaStrategis: number; totalHargaJual: number;

  // --- Nilai Rumah ---
  nrBiayaKprAsuransi: number; nrDiskonAngsuran: number; nrDiskonCash: number; nrBiayaBbn: number;
  nrBiayaNotarisAjb: number; nrBiayaAppraisal: number; nrBiayaBphtb: number; nrLainLain: number;
  nrTotalSubsidi: number; nrNilaiPenyerahan: number; nrPpn: number; nrBphtb: number; nrPph: number;

  // --- Pajak ---
  pjBiayaKpr: number; pjBiayaAsuransi: number; pjDiskonAngsuran: number; pjBiayaBbn: number;
  pjBiayaAjb: number; pjBiayaAppraisal: number; pjBphtb: number; pjLainLain: number;
  pjTotalSubsidi: number; pjNilaiPenyerahan: number; pjPpn: number; pjBphtbPajak: number;
  pjPph: number; pjTotalBphtbPph: number;

  // --- Keperluan AJB ---
  ajbNjopTanahPerMeter: number; ajbNjopTanah: number; ajbNjopBangunanPerMeter: number;
  ajbNjopBangunan: number; ajbNjopTotal: number; ajbPpn: number; ajbBphtb: number;
  ajbPph: number; ajbTotalBphtbPph: number; ajbSelisihPajakPbb: number; ajbUping: number;
}
...
const Kavling = () => {
  const [data, setData] = useState<KavlingData[]>([]);
  ...
  // Komponen Helper untuk memisahkan section form
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {activeTab === 'pajak' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="md:col-span-2 mt-2 p-4 bg-gray-600 rounded-md flex justify-between items-center">
                <span className="text-sm font-medium">BPHTB + PPh</span>
                <span className="font-bold">{formatRupiah(formData.pjTotalBphtbPph)}</span>
              </div>
            </div>
          )}
          ...
          {activeTab === 'ajb' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="md:col-span-2 mt-2 p-4 bg-gray-600 rounded-md flex justify-between items-center">
                <span className="text-sm font-medium">BPHTB + PPh (AJB)</span>
                <span className="font-bold">{formatRupiah(formData.ajbTotalBphtbPph)}</span>
              </div>
