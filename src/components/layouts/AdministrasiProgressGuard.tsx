...
interface AdministrasiProgressGuardProps {
  children: ReactNode;
}

const AdministrasiProgressGuard = ({ children }: AdministrasiProgressGuardProps) => {
  const { user } = useAuth();
  const canAccess =
    canReadResource(user, 'CUSTOMER') || canReadResource(user, 'PROGRESS_PENJUALAN');
...
export default AdministrasiProgressGuard;
