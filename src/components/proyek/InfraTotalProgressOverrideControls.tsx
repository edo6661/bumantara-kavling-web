...
import {
  ...
  useResetTotalProgressBySpk,
  useSetTotalProgressBySpk,
} from '../../hooks/queries/useProgressProyek';
import { handleApiError } from '../../utils/errorHandler';

const clampPercent = (value: number) => Math.min(100, Math.max(0, value));

interface InfraTotalProgressOverrideControlsProps {
  spkId: number;
  fallbackProgress?: number;
  fallbackIsOverride?: boolean;
  canEdit: boolean;
  compact?: boolean;
}

const InfraTotalProgressOverrideControls = ({
  spkId,
  fallbackProgress = 0,
  fallbackIsOverride = false,
  canEdit,
  compact = false,
}: InfraTotalProgressOverrideControlsProps) => {
  const { data, isLoading } = useGetProgressInfraDetail(spkId);
  const setTotalMutation = useSetTotalProgressBySpk();
  const resetTotalMutation = useResetTotalProgressBySpk();

  const isOverride = data?.progress.persentaseIsOverride ?? fallbackIsOverride;
  const current = data?.progress.persentase ?? fallbackProgress;
      ...
      <div className="flex items-center gap-2">
        ...
        {!compact && (
          <span className="text-[10px] text-slate-500">
            {isOverride ? '(manual)' : '(default dari item pekerjaan)'}
          </span>
        )}
      </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                await setTotalMutation.mutateAsync({ spkId, persentase: parsed });
              } catch (err: unknown) {
          ...
          <button
            ...
            onClick={async () => {
              try {
                await resetTotalMutation.mutateAsync({ spkId });
              } catch (err: unknown) {
                alert(handleApiError(err).message);
              }
            }}
...
export default InfraTotalProgressOverrideControls;
